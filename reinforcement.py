...
class Agent(object):
    ...
    def qlearning(self,status,lst_turns):
        ...
        for turnIndex in range(len(lst_turns)-1,-1,-1):            #zaehlt die lsite von hinten nach vorn
            #immer der letzte turn auf tmpEndValue, 
            #da gewonnen oder verlroen
            #dann werden die turns davor berechnet 

            if turnIndex == len(lst_turns)-1: #last turn
                lst_tmpTurnValues[turnIndex] = tmpEndValue
            else:                           #pre turns
                lst_tmpTurnValues[turnIndex] = tmpEndValue

            if turnIndex >= 0:
                tmpEndValue = self.learnCalculate(tmpEndValue, self.getQTableValue(turnIndex ,lst_turns[turnIndex]) ) #minus 1 weil qtable bei 0 anfangen ist
    ...
    def learnCalculate(self, value, oldCurrentTurnValue):
        value = oldCurrentTurnValue + (value * self.lernrate) 
        value = self.roundNumber(value)
        return value    
